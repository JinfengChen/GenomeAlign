...
foreach $dir(@dir){
        #print "DIR $dir\n";
        $scaf=basename $dir;
        print OUT "/opt/lastz/1.03.02/lastz $dir $cwd/$opt{qry_spec}.fa --strand=both --format=axt --ambiguous=n --ambiguous=iupac K=4500 L=3000 Y=50000 E=150 H=0 O=600 T=2 > $cwd/lastz_axt/$scaf.axt\n";
}
close OUT;
...
sub qsub {
	...
	chomp $scaf_num;
        ...
        foreach $qsub_file(@qsub_file){
                ...
                `qsub -q js -l nodes=1:ppn=1,mem=2G,walltime=100:00:00 $qsub_file`;
        }
        chdir $cwd;
